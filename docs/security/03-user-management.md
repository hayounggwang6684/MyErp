...
- 모든 사용자는 직원 레코드와 연결될 수 있다.
- 하지만 모든 직원이 ERP 사용자 계정을 가지는 것은 아니다.
- 직원 정보와 ERP 계정은 분리된 데이터로 관리한다.
...
- 소속 부서와 권한 역할은 분리해 관리한다.
- 동일 부서가 아니어도 업무상 필요하면 별도 권한 역할을 부여할 수 있다.
...
- Mac mini 로컬 관리자 화면의 `사용자 관리`에서 사용자별 역할을 직접 조정할 수 있다.
- 부서는 조직 소속을 의미하고, 역할은 기능 접근 범위를 의미한다.
- 권한 변경은 감사 로그에 기록한다.

### 11.2 직원과 사용자 계정 분리

- 관리자 로컬 화면의 `사용자 관리`는 `ERP 사용자 계정`과 `직원 기본정보`를 함께 보여준다.
- 직원은 계정 없이 존재할 수 있다.
- 계정 잠금, MFA 초기화, 역할 변경은 사용자 계정 영역에서만 수행한다.
- 부서, 직무, 연락처, 담당 건수 같은 정보는 직원 기본정보 영역에서 관리한다.
