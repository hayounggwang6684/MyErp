...
- `디자인 모드`는 운영 기능이 아니라 개발자가 특정 하위 탭에 임시로 적용하는 디자인 검토용 모드이다.
- 디자인 모드는 앱 안에 토글 버튼을 만들지 않고 코드 플래그 또는 개발 지시로만 활성화한다.
- 디자인 모드 적용 시 대상 workspace 내부 요소만 위치/크기 조정 가능해야 하며, 상위 탭바나 다른 업무 탭에 영향을 주면 안 된다.
